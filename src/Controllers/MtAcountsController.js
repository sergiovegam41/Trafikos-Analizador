...
class MtAcountsController {
    ...
    static async getHistoryTraceabilitySummary(acount_id, SQLClient) {
        ...
        const results = await query("SELECT *, CAST(balance AS FLOAT) - CAST(equity AS FLOAT) AS flotante FROM summary_detail_users WHERE account_id = '" + acount_id + "' order by created_at asc LIMIT 60;");
        // console.log(acount_id)
        // console.log(results)
        return results;
